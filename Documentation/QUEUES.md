...
→ Génération automatique du QrCodeToken (colonne existante)
...
2. Stocker dans la colonne `QrCodeToken` de la table `businesses`
3. Construire l'URL : `https://app.waitify.fr/q/{QrCodeToken}`
...
Elle existe implicitement via les entrées dans `QueueEntries`.
...
6. **API** :
   - Vérifie que `IsQueueActive = true` pour ce business
   - Calcule la prochaine position disponible
   - Insère une nouvelle ligne dans `QueueEntries`
...
La file d'attente = l'ensemble des `QueueEntries` avec `Status = 'Waiting'` pour un `BusinessId` donné.
...
- Vérifier que `IsQueueActive = true`
- Vérifier que la file n'est pas pleine (`MaxQueueSize`)
