...
- Les tables sont au pluriel et en minuscule (exemple : `users`), Supabase force cela
...
- `max_businesses` : -1 pour illimité, sinon nombre maximum d'établissements


- `features` : Fonctionnalités JSON pour personnaliser l'expérience selon le plan


- `sms_quota_monthly` : Quota SMS inclus dans l'abonnement
...
## Row Level Security (RLS) multi-business
...
-- Politiques sécurisées multi-business
...
CREATE POLICY "Users access queues via businesses" ON queue_entries
    ...
    FOR ALL USING (auth.uid() = (SELECT UserId FROM businesses WHERE id = BusinessId));
...
CREATE POLICY "Users access SMS logs via businesses" ON sms_logs
    ...
    FOR SELECT USING (auth.uid() = (SELECT UserId FROM businesses WHERE id = BusinessId));
...
## Index et contraintes avancées multi-business
...
-- Index composites optimisés pour le multi-business


CREATE INDEX idx_queue_entries_user_business_status ON queue_entries((SELECT UserId FROM businesses WHERE id = BusinessId), BusinessId, status);


CREATE INDEX idx_sms_logs_user_business_date ON sms_logs((SELECT UserId FROM businesses WHERE id = BusinessId), BusinessId, sent_at);


CREATE INDEX idx_analytics_daily_user_date ON analytics_daily((SELECT UserId FROM businesses WHERE id = BusinessId), date DESC);





-- Index partiels pour optimiser les requêtes actives
...
-- Contraintes métier pour le multi-business
...
-- Contrainte unique pour éviter les doublons de QR codes


CREATE UNIQUE INDEX idx_businesses_qr_token_unique ON businesses(qr_code_token) WHERE is_active = true;
...
BEGIN
    ...
    LOOP
        ...
        IF user_record.business_name IS NOT NULL THEN
            ...
            INSERT INTO businesses (
                ...
                COALESCE(user_record.business_type, 'other'),
                ...
                COALESCE(user_record.country, 'France'),
                ...
                CASE
                    ...
                    WHEN user_record.business_type IS NOT NULL THEN


                        (SELECT (value::jsonb->>user_record.business_type)::integer


                         FROM system_configs WHERE key = 'default_service_times')
...
BEGIN
    ...
    WHERE created_at < NOW() - INTERVAL '%d days' USING days_old
...
Cette architecture multi-business complète permet une gestion flexible et scalable de plusieurs établissements par utilisateur, avec une facturation consolidée, des analytics comparatives et une sécurité robuste via RLS.
